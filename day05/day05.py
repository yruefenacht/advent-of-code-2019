    ...
    def process_codes(self, id):
        ...
        while ip < len(self.ops):
            ...
            if op == 2:
                ...
                ip += 4
            if op == 3:
                self.ops[self.ops[ip+1]] = input_value
                ip += 2
            if op == 4:
                par1 = self.ops[self.ops[ip + 1]] if mode1 == 0 else self.ops[ip + 1]
                print(par1)
            ...
            if op == 5:
                par1, par2 = self.set_param(ip, mode1, mode2)
                ip = par2 if par1 != 0 else ip + 3
            if op == 6:
                par1, par2 = self.set_param(ip, mode1, mode2)
                ip = par2 if par1 == 0 else ip + 3
            if op == 7:
                par1, par2 = self.set_param(ip, mode1, mode2)
                self.ops[self.ops[ip + 3]] = 1 if par1 < par2 else 0
                ip += 4
            if op == 8:
                par1, par2 = self.set_param(ip, mode1, mode2)
                self.ops[self.ops[ip + 3]] = 1 if par1 == par2 else 0
...
# PART 1
intCode.process_codes(1)

# PART 2
intCode.process_codes(5)
